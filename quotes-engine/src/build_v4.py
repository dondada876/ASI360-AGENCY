...
OUT_DOCX = "/Users/dbucknor/Downloads/360_Quotes_Engine/MadOak_NVR_Upgrade_Proposal_v4_March2026.docx"
...
# ── Add header (every page except first) ──────────────────────────────────────
...
ftr_run = ftr_para.add_run("ASI 360  |  415-874-7979  |  don@asi360.co  |  asi360.co")
...
body(doc, "415-874-7979  |  don@asi360.co  |  asi360.co")
...
r = p_co.add_run("ASI 360 | Allied Systems Integrations\n415-874-7979 | don@asi360.co | asi360.co")
...
# ══════════════════════════════════════════════════════════════════════════════
# PAGE 4+ — SCOPE OF WORK (Security Plan)
# ══════════════════════════════════════════════════════════════════════════════
page_break(doc)
...
product_card(doc,
    ...
    "Installed at the dining area entry. Each future Luminys camera you add expands AI search across more of your fleet."
)
spacer(doc, 4)
...
phases = [
    ("Phase 1: Conception & Initiation  |  March 9, 2026", [
        ("1.1  Project Charter", "Contract executed, deposit received, project officially launched. Scope of work confirmed with Daniel."),
    ...
    ("Phase 2: Definition & Planning  |  March 9–14, 2026", [
        ...
        ("2.2  Budget Approval", "50% deposit confirms NVR order. Luminys R54-32NA shipped from Texas — 5 business day lead time, estimated arrival March 16."),
        ("2.3  Work Breakdown Schedule", "Installation day confirmed: Monday, March 23, 2026. Backup date: Wednesday, March 25. Day-of task list and technician schedule finalized."),
    ...
    ("Phase 3: Launch & Execution  |  March 16–22, 2026", [
        ...
        ("3.3  Quality Assurance", "ONVIF/RTSP protocol compatibility verified for each camera model in inventory. Legacy 4K, IMAX series, and new Luminys unit all confirmed."),
...
for opt_label, opt_color, line_items, subtotal, discount, total in [
    ("Option A — 2-Week Storage (8TB)", "003B71",
     [
         ...
         ("Luminys N3T-8LA2, 8MP Mini Turret, LumiLuxSmart, IR + White-light (Dining Area)", "1", "$324.00", "$324.00"),
    ...
    ("Option B — 1-Month Storage (16TB) — Recommended", "1A6E1A",
     [
         ("Everything in Option A (subtotal)", "—", "—", "$4,000.00"),
         ("UPGRADE: Additional 8TB Surveillance HDD (dual-drive config)", "1", "$175.00", "$175.00"),
         ("UPGRADE: Dual-drive RAID configuration & verification", "1", "$75.00", "$75.00"),
     ],
     "$4,250.00", "-$500.00", "$3,750.00"),
]:
    tbl_hdr = doc.add_table(rows=1, cols=1)
    cell = tbl_hdr.rows[0].cells[0]
    ...
    tbl = doc.add_table(rows=len(line_items)+3, cols=4)
    tbl.style = 'Table Grid'
    col_hdrs = ["Line Item", "Qty", "Unit Price", "Total"]
    ...
    for ri, (desc, qty, unit, tot) in enumerate(line_items):
        for ci, val in enumerate([desc, qty, unit, tot]):
            cell = tbl.rows[ri+1].cells[ci]
            r = cell.paragraphs[0].add_run(val)
            r.font.size = Pt(9)
            if ri % 2 == 0: set_cell_shading(cell, "F8F8F8")

    # Subtotal row
    sr = len(line_items)+1
    tbl.rows[sr].cells[2].paragraphs[0].add_run("SUBTOTAL").font.size = Pt(9)
    tbl.rows[sr].cells[3].paragraphs[0].add_run(subtotal).font.size = Pt(9)
    tbl.rows[sr].cells[2].paragraphs[0].runs[0].bold = True

    # Discount + Total
    tr = len(line_items)+2
    tbl.rows[tr].cells[2].paragraphs[0].add_run("ASI 360 Client Discount").font.size = Pt(9)
    tbl.rows[tr].cells[3].paragraphs[0].add_run(discount).font.size = Pt(9)
    tbl.rows[tr].cells[2].paragraphs[0].runs[0].bold = True

    # Insert TOTAL row manually
    from docx.oxml import OxmlElement as OE
    tot_row = OE('w:tr')
    for ci2, val2 in enumerate(["", "", f"OPTION TOTAL", total]):
        tc = OE('w:tc')
        tcp = OE('w:tcPr')
        if ci2 == 2 or ci2 == 3:
            shd = OE('w:shd'); shd.set(qn('w:val'),'clear'); shd.set(qn('w:fill'), opt_color)
            tcp.append(shd)
        tc.append(tcp)
        p_el = OE('w:p')
        r_el = OE('w:r')
        ...
        b_el = OE('w:b')
        rPr.append(b_el)
        sz = OE('w:sz'); sz.set(qn('w:val'), '20'); rPr.append(sz)
        if val2 == total or val2.startswith("OPTION"):
            color_el = OE('w:color'); color_el.set(qn('w:val'), 'FFFFFF'); rPr.append(color_el)
        r_el.append(rPr)
        t_el = OE('w:t'); t_el.text = val2; r_el.append(t_el)
        p_el.append(r_el); tc.append(p_el); tot_row.append(tc)
    tbl._tbl.append(tot_row)
    spacer(doc, 6)

body(doc, "Option A is priced at $3,500 net after a $500 ASI 360 client discount (pre-discount: $4,000). Option B with 1-month retention is $3,750 net (pre-discount: $4,250). Remaining budget can be applied toward camera refurbishment repair parts pending on-site diagnosis.", size=9, italic=True)
...
for ri, row_data in enumerate(roadmap):
    for ci, val in enumerate(row_data):
        cell = tbl.rows[ri+1].cells[ci]
        r = cell.paragraphs[0].add_run(val)
        r.font.size = Pt(9)
...
terms = [
    "50% deposit required to schedule and procure hardware",
    "Balance due upon completion and client sign-off",
    "90-day labor warranty on all configured settings and IP assignments",
    "Hardware manufacturer warranty applies to NVR and cameras (1–3 years depending on brand)",
...
r2 = p_opt.add_run("☐ Option A — $3,500   |   ☐ Option B — $3,750")
r2.font.size = Pt(10)
...
for ri, (left_label, right_label) in enumerate(sig_data):
    ...
    rr = rc.paragraphs[0].add_run(right_label if ri == 1 else right_label)
...
# White text BoldSign tags (invisible to reader, parsed by BoldSign PDF processor)
...
doc.add_paragraph()
body(doc, "Questions? Contact Don Bucknor  |  415-874-7979  |  don@asi360.co  |  asi360.co", size=9, color=GRAY, align=WD_ALIGN_PARAGRAPH.CENTER)
